...
class hdmiCec extends eqLogic {
    ...
    */
	...
	public static function health() {
		$return = array();
		$statusDaemon=false;
		$statusDaemon = config::byKey('daemon','hdmiCec');
	...
	public static function deamon_info() {
		...
		$return['state'] = 'nok';
		$count = trim( shell_exec ('ps ax | grep "cecHdmi_server" | grep -v "grep" | wc -l') );
		if ($count >= 1) {
			$return['state'] = 'ok';
		}
	...
	public static function deamon_start($_debug = false) {
		...
		log::add('hdmiCec', 'info', 'Lancement du daemon cecHdmi');

		$cmd = realpath(dirname(__FILE__)).'/../../resources/cecHdmi_server.py';
		log::add('hdmiCec', 'debug', 'nom complet du daemon cecHdmi : ' . $cmd);
		//TODO il faut lancer le serveur hdmi sur la machine Ip définie, pas uniquement en local
		$cmd = 'nice -n 19 /usr/bin/python ' . realpath(dirname(__FILE__)).'/../../resources/cecHdmi_server.py '.config::byKey('hdmiCecPort', 'hdmiCec', '0').' '.config::byKey('hdmiCecOsdName', 'hdmiCec', 'Jeedom').' '.config::byKey('deviceCecType', 'hdmiCec', '0').' '.config::byKey('internalAddr', 'core', 'xxx.yyy.zzz.vvvv').' '.config::byKey('api', 'core', 'xxxxxxxxx');
		...
		if ($i >= 30) {
			log::add('hdmiCec', 'error', 'Impossible de lancer le daemon cecHdmi, vérifiez les logs', 'unableStartDeamon');
			return false;
		}
		...
		log::add('hdmiCec', 'info', 'cecHdmi lancé');
		return true;
	}
	...
	public static function synchronisation() {
		...
		if (self::deamon_info()['state']=='nok') return "Il faut démarrer le serveur pour pouvoir lancer une synchronisation.";
		$resource_path = realpath(dirname(__FILE__) . '/../../resources');
		sleep(1);
	...
	public static function detection() {
		...
		if (self::deamon_info()['state']=='nok') return "Il faut démarrer le serveur pour pouvoir lancer une détection.";
		$count = 0;
		$opts = array(
				'http'=>array(
						'method'=>"GET",
		...
		//log::remove('squeezeboxcontrol_update');
	}
	...
	public static function event() {
		...
		foreach ($event as $key => $value) {
			if ($key == 'logicalAddress') continue;
			...
			if (is_object($cmd)) {
					...
					if ($key != 'status') {
						if ($cmd = $eqLogic->getCmd ( null, 'status' )) {
							...
							if ($cmd->execCmd ( null, 2 ) == self::convertStatus ( 'Off' )) {
								...
								log::add ( 'hdmiCec', 'debug', 'On passe par déduction l\'équipement de Off à Standby puuisqu\'il dialogue. Il est erroné de penser qu\'il est on. On pourrait lancer un polling pour lever le doute.' );
								$cmd->event ( self::convertStatus ( 'Standby' ) );
								$changed = true;
								
								// Est-ce qu'il a un equipement lié à passer en standby ?$eqType_name = 'hdmiCec';
								$configuration = '"physicalAddress":"' . $eqLogic->getConfiguration ( "physicalAddress" ) . '"';
								$eqLogicBounds = eqLogic::byTypeAndSearhConfiguration ( 'hdmiCec', $configuration );
								foreach ( $eqLogicBounds as $eqLogicBound ) {
									if ($eqLogic->getName () == $eqLogicBound->getName ())
				...
				if ((($key == 'status') and ($value == self::convertStatus('Standby')) and (strpos($eqLogic->getLogicalId(),'Tuner')===false))) {
					...
					$eqLogicBounds = eqLogic::byTypeAndSearhConfiguration ( 'hdmiCec', $configuration );
					foreach ( $eqLogicBounds as $eqLogicBound ) {
						if ($eqLogic->getName() == $eqLogicBound->getName()) continue;
    ...
    public function initStackData() {
        ...
        nodejs::pushUpdate('hdmiCec::initStackDataEqLogic', array('plugin' => 'hdmiCec', 'action' => 'saveStack', 'callback' => 'displayEqLogic'));
    }
    ...
    public function postSave() {
	    ...
	    if (($this->getConfiguration('osdName') == config::byKey('hdmiCecOsdName', 'hdmiCec', 'Jeedom')) or
	    	(in_array($this->getConfiguration('logicalAddress'),array('Audio','Recorder 1','Recorder 2','Recorder 3','Tuner 1','Tuner 2','Tuner 3')))) {
