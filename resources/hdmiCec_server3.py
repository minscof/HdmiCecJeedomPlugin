...
class jeedomRequestHandler(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        ...
        if not cmd:
            self.start_response('200 OK', "text/html", '<h1>Welcome. Try a command ex : scan, stop, start.</h1>')
            return
        ...
        if cmd == 'scan':
            ...
            print("data =", data)
            content_type = "text/javascript"
            self.start_response('200 OK', "text/javascript", data)
            return
        ...
        if cmd == 'dump':
            ...
            content_type = "text/javascript"
            self.start_response('200 OK', "text/javascript", '{"result":"ok"}')
            return
...
class pyCecClient:
  ...
  def MainLoop(self):
    ...
    print("EVENT to notify send command", jeedomCmd + value)
    self.logger.debug('notify jeedom ', jeedomCmd + value)
    urllib.request.urlopen(jeedomCmd + urllib.parse.quote(value)).read()
